...
module.exports = {
        ...
        show: function(req, res) {
            var id = req.param('id');
            Entity.findOne(id).populate('fields').exec(function(err, entityshow) {
        ...
        create: function(req, res) {
                ...
                Entity.create(params, function(err, create) {
                        if (err) return res.send(err, 500)
                        var id = create.id;
                        Entity.find({ id: id }).populate('fields').exec(function(err, entitycreate) {
                            if (err) return res.send(err, 500);

                            // console.log(create);
                            res.json({ entitys: entitycreate });
                           });
                        });
                    },
